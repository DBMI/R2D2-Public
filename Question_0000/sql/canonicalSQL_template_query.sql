...
Question in Text: Comparison of local institutional COVID patient identification to the proposed R2D2 conceptset
Database: SQL Server
...
Section 1: Site specific customization required
	...
	2) Identify COVID positive patients using local institutional methods (Registry etc).
	3) Local PCR tests: identify PCR tests with positive results.

Section 2: Create R2D2 conceptsets
	1)	have a positive PCR test (R2D2 conceptset #) 
	2)	have a positive combination diagnosis code. Different codes for before and after 04/01/2020 during IP/EI hospitalization
...
**************************************************************************************************************/
	...
	into #registry 
	from OMOP5.cohort where cohort_definition_id  =100200  ---Site10 COVID-19 CONFIRMED POSITIVE REGISTRY
	

	--PCR tests
	drop table if exists #PCR 
	select M.person_id, m.measurement_concept_id, m.measurement_datetime, m.value_source_value
	--, m.value_as_concept_id, c.concept_name [value_as_concept_text]
	into #PCR
	from OMOP5.measurement m
	left join vocab5.concept c on c.concept_id = m.value_as_concept_id 
	where m.measurement_concept_id = 706163				-- Site 10 PCR tests
	and m.value_as_concept_id in (45877985, 45884084)	-- Site 10 lab test values (detected, positive)
...
left join vocab5.CONCEPT c on c.concept_id = m.concept_id  
...
**************************************************************************************************************/
	...
	from (
		select distinct m.person_id, m.visit_occurrence_id, m.condition_concept_id, m.condition_source_value, m.condition_start_date
		from omop5.condition_occurrence m 
	...
	join (
		...
		from OMOP5.condition_occurrence c
	...
	select  m.person_id, m.measurement_concept_id, m.value_source_value, m.measurement_date
	into #R2D2 
	from #conceptset242 c																			--- R2D2 non-pre-coordinated viral lab tests
	join OMOP5.measurement m on c.concept_id = m.measurement_concept_id
	...
	join OMOP5.measurement m on c.concept_id = m.measurement_concept_id
	union 
	select m.person_id, m.condition_concept_id, m.condition_source_value, m.condition_start_date
	from #conceptset248 c																			-- R2D2 Diagnosis codes without logic
	join OMOP5.condition_occurrence m on c.concept_id = m.condition_concept_id
	union
	select distinct m.person_id, m.condition_concept_id, m.condition_source_value, m.condition_start_date
	from #cond m																					--- R2D2 diagnosis codes with logic 
	


	---Comparision of internal registry, PCR vs R2D2
	drop table if exists #reg_comp_R2D2
	select registry,  PCR, R2D2, count(distinct person_id) PatientCounts
	into #reg_comp_R2D2
	from (	select a.person_id
	, case when r.person_id is not null then 1 else 0 end as [registry]
	, case when pcr.person_id is not null then 1 else 0 end as PCR
	, case when R2D2.person_id is not null then 1 else 0 end as [R2D2]
	from (select distinct person_id from #registry 
			union 
			select distinct person_id from #PCR 
			union
			select distinct person_id from #R2D2 
	) a 
	full outer join #registry r on r.person_id = a.person_id
	full outer join (select distinct person_id from #PCR) pcr on pcr.person_id = a.person_id
	full outer join (select distinct person_id from #R2D2) R2D2 on R2D2.person_id = a.person_id
	) a
	group by registry, PCR, R2D2
	order by registry desc , PCR desc, R2D2 desc 

	
	-- create full set of possible values
	drop table if exists #PV 
	select 1 registry, 1 PCR, 1 R2D2 into #PV 
	union 
	select 1 registry, 1 PCR, 0 R2D2  
	union 
	select 1 registry, 0 PCR, 1 R2D2  
	union 
	select 1 registry, 0 PCR, 0 R2D2  
	union 
	select 0 registry, 1 PCR, 1 R2D2  
	union 
	select 0 registry, 1 PCR, 0 R2D2  
	union 
	select 0 registry, 0 PCR, 1 R2D2  
...
**************************************************************************************************/
	...
	, case when pv.registry = 1 then 'yes' else 'no' end as PrivateReference_PresentIn_InsitutionalRegistry
	, case when pv.PCR = 1 then 'yes' else 'no' end as UniversalReference_HasLabResults_PCRTestPositive
	, case when pv.R2D2 = 1 then 'yes' else 'no' end as ProposedMethod_PresentIn_R2D2CanonicalSQL
	, case when r.PatientCounts is null then 0 else r.PatientCounts end as PatientCount
	, @version as QueryVersion
	, @queryExecutionDate as QueryExecutionDate
	from #PV pv 
	left join #reg_comp_R2D2 r on r.registry = pv.registry 
		and r.PCR = pv.PCR and r.R2D2 = pv.R2D2	
	order by pv.registry desc, pv.PCR desc, pv.R2D2 desc 
