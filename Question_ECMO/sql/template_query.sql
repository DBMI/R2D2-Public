...
if object_id('tempdb.dbo.#covid_hsp') is null 
begin
	...
	create table #covid_hsp (
			...
			, visit_start_datetime datetime2(7)
			, visit_end_datetime datetime2(7)
			, discharge_to_concept_id bigint
			, discharge_to_source_value varchar(256)
			, AgeAtVisit int
			, cohort_start_date datetime2(7)
			, days_bet_COVID_tst_hosp int
			, death_datetime datetime2(7)
	...
	exec sp_identify_hospitalization_encounters	--- gets COVID hospitalizations

end 

----------------------------------------------------------------------------------------------------------------------------------------------------------			

declare @sitename varchar(20) = 'Site12'
declare @version numeric(2,1) = 3.1;
declare @queryExecutionDate datetime = (select getdate());
declare @minAllowedCellCount int = 0;
declare @maskLabel varchar(20) = '[1-10]';
...
select * into #ecmo from  omop.dbo.concept where CONCEPT_ID IN 
...
select distinct * into #covid_hsp_ecmo 
from
	(
		...
		from omop.dbo.condition_occurrence c
		...
		from omop.dbo.procedure_occurrence c
		inner join  #covid_hsp s on c.person_id =  s.person_id
		where 
...
--Create Labels
	...
	into #gender from omop.dbo.CONCEPT 
...
if object_id('tempdb.dbo.#race') is not null drop table #race  
	select concept_id covariate_id, domain_id covariate_name, concept_name covariate_value  
	into #race from omop.dbo.CONCEPT 
...
if object_id('tempdb.dbo.#ethnicity') is not null drop table #ethnicity  
	select concept_id covariate_id, domain_id covariate_name, concept_name covariate_value  
	into #ethnicity from omop.dbo.CONCEPT 
...
when datediff(dd, p.birth_datetime, getdate())/365 is NULL then 'Unknown'
	...
	inner join omop.dbo.PERSON p on p.PERSON_ID  = h.person_id
